...
public class PlayerWrath_ZachyNati : MonoBehaviour
{
    ...
    PlayerWrath_Combat combatState;
    ...
    void Start()
    {
        ...
        combatState = GetComponent<PlayerWrath_Combat>();
    }
    ...
    void setUsername(string Player)
    {
        try
        {
            GameObject.Find(Player).GetComponent<PlayerWrath_ZachyNati>().username.text = Player;
            GameObject.Find(Player).GetComponent<PlayerWrath_ZachyNati>().username.transform.LookAt(GameObject.Find("Main Camera").transform);
            GameObject.Find(Player).GetComponent<PlayerWrath_ZachyNati>().username.transform.rotation = Quaternion.LookRotation(GameObject.Find("Main Camera").transform.forward);
        }
    ...
    void Jumpy(string player, Vector3 vel, bool pls)
    {
        try
        {
            if (pls)
            {
                ...
                GameObject.Find(player).GetComponent<PlayerWrath_ZachyNati>().isJumping = true;
            }
            else
            {
                GameObject.Find(player).GetComponent<Rigidbody>().velocity += vel;
                GameObject.Find(player).GetComponent<PlayerWrath_ZachyNati>().isJumping = false;
            }
        }
        catch (NullReferenceException e)
        {
    ...
    IEnumerator Stunah(int value)
    {
        ...
        stunTheBitch = false;
        GetComponent<PlayerWrath_Combat>().theBitchIsStunned = false;
    }
