...
public class AnimationWrath_ZachyNati : MonoBehaviour
{
    PhotonView view;
    ...
    private PlayerWrath_ZachyNati movementScript;
    ...
    void Start()
    {
        ...
        if (view.IsMine)
        {
            movementScript = GetComponent<PlayerWrath_ZachyNati>();
    ...
    void FixedUpdate()
    {
        if (view.IsMine && !GetComponent<PlayerWrath_ZachyNati>().stunTheBitch) //disabling punching and running for any testing characters.
        {
    ...
    void Pullies(string player, bool state)
    {
        try
        {
            GameObject.Find(player).GetComponent<PlayerWrath_Combat>().isPulling = state;
        }
